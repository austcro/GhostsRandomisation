...
{
    public class MSExcel : BaseHandler, IExcel
    {
        ...
        {
            try { 
            EmulateExcelAction(timeline, handler);
                return true;
            }
            catch(Exception ex)
            {
        ...
        private void EmulateExcelAction(Timeline timeline, TimelineHandler handler)
        {
            ...
            try
            {
                foreach (TimelineEvent timelineEvent in handler.TimeLineEvents)
                {
                    try
                    {
                        ...
                        ExcelApplicationData excelApplicationData = excelApplication.CreateExcelApplication();

                        var writeSleep = ProcessManager.Jitter(100);
                        Thread.Sleep(writeSleep);

                        excelApplicationData.ExcelWorkSheet.Cells[1, 1].Value = "We have 2 simple shapes created.";
                        ...
                        excelApplicationData.ExcelWorkBook.SaveAs(path);
                        ...
                        excelApplicationData.ExcelApplication.Quit();
                        excelApplicationData.ExcelApplication.Dispose();
                        excelApplication = null;

                        excelApplicationData.ExcelWorkBook = null;
                        excelApplicationData.ExcelWorkSheet = null;

                        try
                        {
                            //Marshal.ReleaseComObject(excelApplication);
                            MarshalProvider.MarshalProvider.ReleaseComObject(excelApplication);
                        }
                        catch { }
                        ...
                        try
                        {
                            ...
                            MarshalProvider.MarshalProvider.FinalReleaseComObject(excelApplication);
                        }
                        catch { }
