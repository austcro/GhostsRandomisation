...
using Ghosts.ConfigProvider;

namespace Ghosts.Infrastructure
{
    /// <summary>
    ...
    public static class FileListing
    {
        ...
        {
            //check if flushing
            if (ApplicationConfigProvider.Configuration.OfficeDocsMaxAgeInHours == -1)
                return;
                ...
                try
                {
                    ...
                    using (var reader = new StreamReader(_fileName))
                    {
                        ...
                        while ((line = reader.ReadLine()) != null)
                        {
                            ...
                            if (file.Exists && (creationTime > ApplicationConfigProvider.Configuration.OfficeDocsMaxAgeInHours)) //clean up and delete files older than x hours
                            {
                                try
                                {
